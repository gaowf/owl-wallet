...
public class GeetestLib {
    ...
    private String addParams(String param, HashMap<String, String> data) {
        String customer_url = data.get("customer_url");
        String customer_ua = data.get("customer_ua");
        if (customer_url != null) {
            try {
                param = param + "&customer_url=" + URLEncoder.encode(customer_url, "UTF-8");
        ...
        if (customer_ua != null) {
            try {
                param = param + "&customer_ua=" + URLEncoder.encode(customer_ua, "uTF-8");
            } catch (UnsupportedEncodingException e) {
                log.error("e:{}", ExceptionUtils.getStackTrace(e));
            }
        }
    ...
    public int enhencedValidateRequest(String challenge, String validate, String seccode, HashMap<String, String> data) {
        ...
        try {
            ...
            // 极验模型分析得出的⼈人⼯工打码的判别，1可能为⼈人⼯工打码
            String model_probability = return_map.getString("model_probability");
            // 极验收集到的⽤用户IP、UA、URL相关信息
            String origin_info = return_map.getString("origin_info");
            // 上述特定参数是否⼀一致
            String uniformity = return_map.getString("uniformity");
            // Ajax的时间减去get的时间
            String duration = return_map.getString("duration");
            logger.info("model_probability : {}", model_probability);
            logger.info("origin_info : {}", origin_info);
            logger.info("uniformity : {}", uniformity);
            logger.info("duration : {}", duration);
            if (model_probability != null && "1".equals(model_probability)) {
//				logger.info("极验行为入库: {}", ipAddress);
//				AbnormalRegisterRecord abnormal = new AbnormalRegisterRecord();
//				abnormal.setCellPhoneNumber("");
//				abnormal.setCtime(Date.from(Instant.now()));
//				abnormal.setEmail("");
//				abnormal.setEmailSuffix("");
//				abnormal.setIp(ipAddress);
//				// 默认值 注册类型 1:手机 2:email 目前极验未传 手机或email
//				abnormal.setType((byte)1);
//				abnormal.setMemo("极验：model_probability ->" + model_probability
//				 + " duration -> " + duration);
//				AbnormalUtil.add(abnormal);
            }
