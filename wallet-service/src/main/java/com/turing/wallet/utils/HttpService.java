...
public class HttpService {
    ...
    public String getRequestSign(String url, Map<String, String> map) {
        String responseBody = "";
        HttpEntity requestEntity = null;
        try {
            MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
            headers.add("exch-id", "1");

            requestEntity = new HttpEntity(toMultiValueMap(map), headers);
            ResponseEntity<String> responseEntity = retryRestTemplate.exchange(url, HttpMethod.GET, requestEntity, String.class);
            responseBody = responseEntity.getBody();
        } catch (Exception e) {
            log.error("url: " + url, e);
            throw new RuntimeException("请求上游失败");
